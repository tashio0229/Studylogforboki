...
| "学び記録 [内容]" / "lesson [text]" | `lessons/` に学び・修正・成功パターンを1件1ファイルで記録。同テーマの既存ノートがあれば新規作成せず更新する |
...
### 経理・財務担当
- todos, meetings, projects, finances, research, knowledge, clients
- おすすめ追加: journal, reading-list
...
- 管理モードでは必ず最初に `.secretary/CLAUDE.md` を読み込む。存在すれば `lessons/` の既存ノート（特に `status: confirmed`）にも目を通し、過去の学びを踏まえて対応する
- 長時間・自律実行時は `.secretary/CLAUDE.md` の「秘書の運用ポリシー」に従う（十分な情報があれば動く／進捗は実ファイルの証拠に基づいて報告／止まるのは本人にしか決められない時だけ／結論から短く）
